...
**order** function is helpful to find the indexes of these cases, or you can re-sort the data by the depth.)
...
load("COOzonehourly_44201_2021.rda")
library( lubridate)
Golden<- rbind( c(-105.2211, 39.754185) )
I<- which.min( rdist.earth(Golden, COOzoneInfo[,1:2]) )
GoldenID<- rownames(COOzoneInfo)[I]
GOzone2021<- 
  data.frame( O3=100*COOzone[,GoldenID],
              date= COOzone$date,
              hour= hour(COOzone$date ),
              day=  yday(COOzone$date  ),
              month = month( COOzone$date)
  )
save( GOzone2021, file="GOzone2021.rda")
```
